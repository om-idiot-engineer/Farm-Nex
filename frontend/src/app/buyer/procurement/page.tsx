...
export default function BuyerProcurementManagerPage() {
  ...
  // Modal form state
  const [commodity, setCommodity] = useState("Soybean (Yellow JS-335)");
  ...
  const filteredRfqs = rfqs.filter((r) => {
    ...
    const commodityStr = r.commodity || r.crop || "";
    const destinationStr = r.deliveryDestination || r.destination || "";
    const matchesSearch = commodityStr.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative max-w-xs w-full">
          ...
          <input
            type="text"
            placeholder="Filter by commodity or plant..."
            value={search}
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-2">
                ...
                <h2 className="text-xl font-black text-foreground">
                  {(rfq.quantityQuintals || rfq.quantity).toLocaleString("en-IN")} Quintals {rfq.commodity || rfq.crop}
                </h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="mt-4 space-y-4">
              <div>
                <label className="text-xs font-bold text-foreground">Commodity & Target Variety</label>
                <input
                  type="text"
                  required
                  value={commodity}
